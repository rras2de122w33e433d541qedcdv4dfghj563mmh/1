...
local function dl_read(chat_id, target)
	local f = io.open("./Turbo/quran/Sura"..target..".mp3","r")
	if f ~= nil then
		io.close(f)
		return send_document("chat#id"..chat_id,"./Turbo/quran/Sura"..target..".mp3", ok_cb, false)
	else
...
local function view_sura(chat_id, target)
	local f = io.open("./Turbo/quran/quran ("..target..").txt","r")
	if f ~= nil then
...
local function run(msg, matches)
	...
	elseif matches [1] == "uran" then
		local file = io.open("./Turbo/quran/list.txt", "r")
